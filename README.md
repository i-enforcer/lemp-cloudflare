...
## Cloudflare ssl certificate installation.

#### Generate Cloudflare ssl certificates and put it them to the location `/etc/ssl/`

### NOTE : After generating ssl certificates keep SSL/TLS encryption mode to `(Flexible)`

#### Add the following configuration to the `/etc/nginx/sites-available/example.com.conf`
...
## Check Nginx Server is running properly on or not.

```
sudo nginx -t
```

## Restart the Ningx Server.

```
sudo systemctl restart nginx
```
### NOTE : After generating ssl certificates keep SSL/TLS encryption mode to `Full (strict)`